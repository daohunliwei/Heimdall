...
""";
        ...
        string languageDisplayName)
    {
        ...
        var fileLinks = string.Join('\n', page.FilePaths.Select(path => $"- [{path}]({BuildRepositoryFileUrl(repoType, repoUrl, repoOwner, repoName, path)})"));
...
""";
    ...
    /// <summary>
    /// 构建单页 Slides HTML 提示词。
    /// </summary>
    public string BuildSlidePrompt(
        string owner,
        string repo,
        string slideTitle,
...
""";
        ...
        return template
            .Replace("__OWNER__", owner, StringComparison.Ordinal)
            .Replace("__REPO__", repo, StringComparison.Ordinal)
            .Replace("__TITLE__", slideTitle, StringComparison.Ordinal)
            .Replace("__INDEX__", slideIndex.ToString(), StringComparison.Ordinal)
            .Replace("__TOTAL__", totalSlides.ToString(), StringComparison.Ordinal)
            .Replace("__DESCRIPTION_LINE__", string.IsNullOrWhiteSpace(slideDescription) ? string.Empty : $"The slide should cover: {slideDescription}", StringComparison.Ordinal)
            .Replace("__WIKI_CONTENT__", wikiContent, StringComparison.Ordinal)
            .Replace("__LANGUAGE__", languageDisplayName, StringComparison.Ordinal);
    }
...
""";
    ...
    /// <summary>
    /// 构建 Slides/Workshop 所需 Wiki 摘要。
    /// </summary>
    public string BuildWikiReferenceText(WikiTaskResponse wikiTaskResponse)
    {
        var builder = new StringBuilder();
        builder.AppendLine($"## Project Overview");
        builder.AppendLine(wikiTaskResponse.WikiStructure.Description ?? string.Empty);
        builder.AppendLine();

        var pages = wikiTaskResponse.WikiStructure.Pages ?? new List<WikiPage>();
        var generatedPages = wikiTaskResponse.GeneratedPages ?? new Dictionary<string, WikiPage>(StringComparer.OrdinalIgnoreCase);
        var totalContentLength = 0;
        const int maxContentLength = 30000;

        foreach (var page in pages.Where(item => string.Equals(item.Importance, "high", StringComparison.OrdinalIgnoreCase)))
        {
            totalContentLength = AppendWikiPage(builder, generatedPages, page, totalContentLength, maxContentLength);
    ...
    private static int AppendWikiPage(
        StringBuilder builder,
        IReadOnlyDictionary<string, WikiPage> generatedPages,
        WikiPage page,
        int totalContentLength,
        int maxContentLength)
    {
        if (!generatedPages.TryGetValue(page.Id, out var generatedPage) || string.IsNullOrWhiteSpace(generatedPage.Content))
        {
            return totalContentLength;
        }

        var fullContent = $"## {page.Title}\n{generatedPage.Content}\n\n";
        if (totalContentLength + fullContent.Length <= maxContentLength)
        {
            builder.Append(fullContent);
            return totalContentLength + fullContent.Length;
        }

        var summary = ExtractSummary(generatedPage.Content);
        var summaryContent = $"## {page.Title}\n{summary}\n\n";
        builder.Append(summaryContent);
        return totalContentLength + summaryContent.Length;
    }
    ...
    private static string GetWikiStructureFormatInstructions(bool isComprehensiveView)
    {
...
""";
    ...
    {
        if (string.Equals(repoType, "local", StringComparison.OrdinalIgnoreCase))
        {
            return filePath;
        }
        ...
        var normalizedRepoUrl = repoUrl.TrimEnd('/').Replace(".git", string.Empty, StringComparison.OrdinalIgnoreCase);
        return repoType switch
        {
            "gitlab" => $"{normalizedRepoUrl}/-/blob/main/{filePath}",
            "bitbucket" => $"{normalizedRepoUrl}/src/main/{filePath}",
            _ => $"{normalizedRepoUrl}/blob/main/{filePath}"
        };
    }
}
