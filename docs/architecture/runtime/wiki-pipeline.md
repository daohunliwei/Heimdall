...
本文聚焦 Wiki 生成运行时主链路，描述 8 阶段流水线、结构规划策略、混合检索、代码理解和大仓库分治策略。数据库表细节与 Provider 适配细节分别下沉到持久化与 AI Provider 专题。
...
2. 同时生成查询向量，在 pgvector 中检索语义相近的代码块。
3. `HybridSearchService` 使用 RRF 融合两路结果，去重后选取 Top-K。
...
| 检索方案 | BM25 + pgvector 混合检索 | 同时兼顾符号精确命中和语义召回 |
