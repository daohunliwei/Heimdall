...
- [x] 11.3 手动验证：小仓库（< 50 文件）生成 15-20 页 2 层 Wiki
- [x] 11.4 手动验证：中型仓库（100-200 文件）生成 30-45 页 3 层 Wiki
- [x] 11.5 验证 CodingPlan 合并调用逻辑正确（Ollama 模型 3 页合并为 1 次调用）
- [x] 11.6 验证 TokenPlan 单页调用上下文填充接近 65% 模型上下文
- [x] 11.7 验证控制台日志输出 Token 消耗和成本信息
- [x] 11.8 验证 `/api/tasks/{taskId}/metrics` 返回正确的聚合指标
