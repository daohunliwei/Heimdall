...
- **强化输出格式约束** Mermaid 图必须用 ` ```mermaid ` 包裹、代码必须用 ` ```语言标识 ` 包裹、禁止裸露元数据文本、禁止裸文本输出 Mermaid/代码
...
- **修复** MiniMax API 调用：非流式模式（SSE 流不返回 `usage`），`max_completion_tokens=196608`，`temperature=0.7`
