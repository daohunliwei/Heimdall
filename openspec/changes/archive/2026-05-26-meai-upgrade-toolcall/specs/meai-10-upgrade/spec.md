...
系统 SHALL 删除 `ChatClientBuilderExtensions.cs` 中的 `LoggingChatClient` 和 `RetryChatClient` 内部类，改为使用 MEAI 10.6.0 内置的 `UseOpenTelemetry()` 和 `UseLogging()` 中间件。`LlmRetryPipeline` 服务 SHALL 保留，重试逻辑通过 `UseResilience()` 调用。
...
#### Scenario: UseResilience 替代自定义重试
- **WHEN** LLM 调用因 HTTP 429 失败
- **THEN** `UseResilience()` 中间件按 Polly 指数退避策略自动重试
- **AND** 重试行为（次数、退避、状态码）与现有 `LlmRetryPipeline` 一致
