...
每个自定义 `IChatClient` 适配器（`OllamaChatClient`、`GeminiChatClient`、`MiniMaxChatClient`）SHALL 实现 10.6.0 `IChatClient` 接口的全部成员，包括新增的 `GetService<T>(object? key)` 方法和 `Metadata` 属性。
...
系统 SHALL 基于 `OllamaSharp` 库实现 `OllamaChatClient : IChatClient`。**变更**：额外实现 10.6.0 新增的 `GetService<T>()` 方法和 `Metadata` 属性；检测并处理 `ChatOptions.Tools`。
...
系统 SHALL 实现 `GeminiChatClient : IChatClient`。**变更**：额外实现 `GetService<T>()`、`Metadata`；支持 Gemini 原生 Function Calling。
...
系统 SHALL 实现 `MiniMaxChatClient : IChatClient`。**变更**：额外实现 `GetService<T>()` 和 `Metadata`。
