...
系统 SHALL 通过 `UseFunctionInvocation(configure: o => { ... })` 配置以下参数：`MaximumIterationsPerRequest` 设为 8（Stage 5 页面生成复杂场景需要更多轮）、`MaximumConcurrentToolCalls` 设为 3（限制并发工具调用数）、禁用 `AllowNonInvokableFunctionCalls`（严格模式）。
...
#### Scenario: 并发工具调用控制
- **WHEN** LLM 单轮请求同时调用 4 个工具（如同时搜索 4 个符号）
- **THEN** `MaximumConcurrentToolCalls = 3` 限制最多 3 个并发执行
- **AND** 第 4 个工具等待前序完成后再执行
...
系统 SHALL 在 `TaskLlmCallLog` 中新增 `ToolCallLogs` 集合属性，记录每次 Tool Call 的工具名、参数（脱敏）、返回长度、耗时。`FunctionInvokingChatClient` SHALL 通过 MEAI 的 `ChatResponse.Choices[].AdditionalProperties` 暴露工具调用详情，系统负责提取并持久化。
...
- **AND** 每条记录包含 `ToolName`、`Arguments`（脱敏后）、`ResultLength`、`DurationMs`、`RoundIndex`
