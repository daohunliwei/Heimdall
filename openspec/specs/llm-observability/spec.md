...
系统 SHALL 通过 `ChatClientBuilder.UseOpenTelemetry()` 中间件自动记录 LLM 调用的 Trace 和 Metrics。原自定义 `LoggingChatClient` 弃用，统一使用 MEAI 内置 `UseOpenTelemetry()` + `UseLogging()` 替代；自定义 `RetryChatClient` 弃用，重试逻辑由 `UseResilience()` 处理。
...
系统 SHALL 在 Provider 响应不包含 token usage 信息时使用 TokenCounter 估算。该规则扩展覆盖流式调用场景，流式完成后统一通过 TokenCounter 估算 `InputTokenCount` 和 `OutputTokenCount`。
...
### Requirement: Tool Call 日志扩展
系统 SHALL 在 `TaskLlmCallLog` 中新增 `ToolCallLogsJson` 属性，记录每次 Tool Call 的工具名、参数、返回长度、耗时和轮次索引。数据从 `ChatResponse` 的 `AdditionalProperties` 中提取。

#### Scenario: Tool Call 日志记录
- **WHEN** Stage 5 页面生成中 LLM 调用 2 次工具
- **THEN** `TaskLlmCallLog.ToolCallLogsJson` 包含格式化的 JSON 记录
