...
系统 SHALL 按 8 阶段执行 Wiki 生成。**变更**：Stage 3（深度代码理解）的 CodeUnderstandingResult 必须作为 Stage 4（结构规划）的核心输入；Stage 5（页面生成）必须根据页面的 ContentDepthLevel 使用差异化提示词；Stage 5 启动前须检查 Debug Mode 开关，开启时按 MaxDebugPages 截断页面列表。
